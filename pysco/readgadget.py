...
# correct all the lines in the file to be compatible in Python3 
import numpy as np
import readsnap
import sys,os,h5py
...
def read_field(snapshot, block, ptype):
    ...
    if fformat == "binary":
        ...
        if block == "POS ":
            ...
            raise Exception(f'block "{block}" not implemented in readgadget!')
        ...
        if full_key not in f:
            if Masses[ptype] != 0.0:
                ...
                raise Exception(f'Problem reading the block "{block}"')
        else:
            array = f[full_key][:]
