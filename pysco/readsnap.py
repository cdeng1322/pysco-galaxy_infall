...
#only used for format I, when file structure is HEAD,POS,VEL,ID,ACCE
  ...
  if parttype >= 0:
    ...
    if data_for_type[parttype]==False:
      priprint("Error: no data for specified particle type", parttype, "in the block", block)
      sys.exit()
  elif block=="MASS":
...
def read_gadget_header(filename):
  ...
  print('CDM mass=%.5e  Omega_CDM = %.5f' % (head.massarr[1]*1e10, Omega_CDM))
  if head.nall[2] > 0 and head.massarr[2] > 0:
      Omega_NU = head.nall[2]*head.massarr[2]*1e10 / (head.boxsize**3 * rhocrit)
      print('NU mass=%.5e  Omega_NU = %.5f' % (head.massarr[2]*1e10, Omega_NU))
      print('Sum of neutrino masses=%.5f eV' % (Omega_NU * head.hubble**2 * 94.1745))
"""
...
# ----- find next block in a GADGET-2 snapshot file -----
def find_next_block(f):
    """
    Read the next GADGET-2 block name and skip over its data.

    Parameters
    ----------
    f : file object
        Already open file, positioned at start of block tag.

    Returns
    -------
    str or None
        Block name (like 'POS ', 'VEL ', etc.), or None if end of file.
    """
    tag_start = f.read(4)
    if len(tag_start) < 4:
        return None  # EOF
    ...
    f.seek(blocksize + 4, 1)  # skip block data + trailing blocksize
...
def find_block(filename, format, swap, block, block_num, only_list_blocks=False):
  format = 2
  target_block = block.encode("utf-8")
  if (not os.path.exists(filename)):
    print("file not found:", filename)
    sys.exit()
  ...
  with open(filename, "rb") as f:
    ...
    curblocksize = None
    if format == 2:
      f.seek(4 + 256 + 4)  # skip 256-byte header + 2x 4-byte size tags
    ...
    while ((not found) and (f.tell()<filesize)):
      ...
      if format==2:
        while True:
          curblock = find_next_block(f)
          print(f"Trying block: {curblock} (looking for {target_block})")

          if curblock is None:
              print("Reached end of file while searching for blocks.")
              break

          if curblock == target_block:
            print("✅ Found target block!")
            #print(f"Trying block: {curblock} (looking for {target_block})")
            found = True
            curblocksize = struct.unpack("i", f.read(4))[0]
          ...
          if found:
            print("✅ Block found!")
            if curblocksize is not None:
                offset = f.tell()
                blocksize = curblocksize

                # ✅ Skip the block content and trailing size marker
                f.seek(blocksize, os.SEEK_CUR)
                print('block size', blocksize)
                f.seek(4, os.SEEK_CUR)  # trailing block size (int32)
                
                break  # exit loop, block was found
            else:
                print("Block was marked as found but curblocksize is undefined.")
                sys.exit()

          elif not found:
            # Skip block correctly (GADGET-2 style): must skip block name tag first
            block_name_size = np.fromfile(f, dtype=np.int32, count=1)[0]  # usually 8
            f.read(4)  # skip name
            block_name_size_end = np.fromfile(f, dtype=np.int32, count=1)[0]

            temp_size = np.fromfile(f, dtype=np.uint32, count=1)[0]  # actual data block size
            f.seek(temp_size, os.SEEK_CUR)
            #blocksize_check = np.fromfile(f, dtype=np.uint32, count=1)[0]
            blocksize_check_arr = np.fromfile(f, dtype=np.uint32, count=1)
            if blocksize_check_arr.size == 0:
                print("DEBUG: Hit end of file while checking block size — likely finished reading blocks.")
                break  # Exit the while loop
            blocksize_check = blocksize_check_arr[0]


            if swap:
                temp_size = temp_size.byteswap()
            f.seek(temp_size, os.SEEK_CUR)
            blocksize_check = np.fromfile(f, dtype=np.uint32, count=1)[0]
            if swap:
                blocksize_check = blocksize_check.byteswap()
            if temp_size != blocksize_check:
                print("DEBUG: Skipping unknown block.")
                print(f"  temp_size = {temp_size}")
                print(f"  blocksize_check = {blocksize_check}")
                print(f"  file position = {f.tell()}")

                print("something wrong: block sizes do not match")
                sys.exit()
